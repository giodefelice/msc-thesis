...
\begin{example}[Kitaev's Toric code]
	...
	If we implement the construction on a simple lattice on a torus, we obtain a quantum storage method. Consider the trivial lattice on a torus with spins on the edges, i.e a square with opposite edges identified as follows:
	\begin{center}
		\begin{tikzpicture}[decoration={markings,mark=at position 0.5 with {\arrow{>}}}]
		\draw (0,0)--(0,1);
		\draw (1,0)--(1,1);
		...
		\end{tikzpicture}
	\end{center}
	This lattice has only one site, states of the system are generated by pairs of elements of $\mathbb{Z}_2$, each labelling one edge of the lattice. Let's say that $L^g_\pm(0)$, $L^g_\pm(1)$ and $T^h_\pm(0)$ $T^h_\pm(1)$ are $L_\pm$ and $T_\pm$ operators acting on the first and second particle. Note that in this configuration, we have only one plaquette and one vertex, we will only need to define two types of operators on the lattice, indexed by elements of $G$. For $g,h \in G$ we have:
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	As for the general case those operators generate the algebra $DG$, and states of the overall system live in a representation of $DG$.
	Note that because we chose our lattice to be trivial, the overall dimension of the system is at most $|G|^2$ and we cannot obtain higher dimensional representations. Even if the system is in a non-trivial representation, we cannot really braid anyons as there is no space to move excitations around each other.
\end{example}
...
As above, we have an oirented lattice on a $2D$ oriented manifold with particles on the edges taking values in $H$. 
...
\begin{theorem}
	If the antipode of $H$ is involutive, then $\mathcal{L}_a$ is a left-left Yetter Drinfeld module.
\end{theorem} 
\begin{proof}
...
Note that if $\mathbb{C}SU(2)$ is an infinite dimensional hopf algebra with involutive antipode. In view of the generalization of Kitaev's model, it is a natural question to ask whether we can apply the same reasoning here, to obtain a model induced by Jordan's model which exhibits topological dependencies. This means applying the Drinfeld center construction to $J_2$ or in other words adding an $SU(2)$ coaction to obtain a theory of $SU(2)$ Yetter-Drinfeld modules. It is not known to the reader whether such model is implementable in practice, but in theory the reasoning works. Indeed, consider
